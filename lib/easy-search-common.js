EasySearch = (function () {
  ...
  function setUpPublication(name, opts) {
    ...
    Meteor.publishComposite(name + '/easySearch', function(conf) {
      return {
        find: function() {
          var resultSet;
          var resultArray;
          ...
          if (!(indexes[name].permission())) {
            throw new Meteor.Error('not-allowed', 'You are not allowed to search this index!');
          }
          ...
          return opts.collection.find({$or: resultArray});
        },
        children: opts.getChildren()
      };
    });
  }
